...
export default function Home() {
  ...
  const interpolatedStreams = useMemo(() => {
    ...
    const deltaStreams = Math.max(
      0,
      Number(data.total_streams || 0) -
        Number(data.yesterday_streams || 0)
    );

    const streams =
      Number(data.yesterday_streams || 0) +
      deltaStreams * progressInDay;

    return Math.floor(streams);
  }, [data, now]);

  // ✅ ДОБАВИЛИ listeners (аналогично streams)
  const interpolatedListeners = useMemo(() => {
    ...
    const deltaListeners = Math.max(
      0,
      Number(data.total_listeners || 0) -
        Number(data.yesterday_listeners || 0)
    );

    const listeners =
      Number(data.yesterday_listeners || 0) +
      deltaListeners * progressInDay;

    return Math.floor(listeners);
  }, [data, now]);

  const currentDistanceKm = Math.floor(
    interpolatedStreams * KM_PER_STREAM
  );
  ...
  if (!data) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-white text-[#0d4b6b]">
        <div className="text-xl tracking-wide animate-pulse">
          Loading...
        </div>
      </main>
    );
  ...
  const remainingKm = Math.max(
    0,
    TOTAL_DISTANCE - currentDistanceKm
  );
          ...
          <div className="bg-white pt-0 pb-6">
            <div className="relative -mt-20 sm:-mt-24 mx-4 sm:mx-8 rounded-[24px] border border-white bg-[#B6DAEB] backdrop-blur-md shadow-[0_8px_25px_rgba(73,107,127,0.15)] px-6 sm:px-8 py-6 sm:py-7">
              <div className="space-y-4 text-[13px] sm:text-[15px] leading-[1.4] text-[#0c4b6a] font-semibold">
                <p>
                  Привет! Это группа “друнк” и мы отправили нашего
                  барабанщика Сашу пробежать вокруг света.
                </p>
                <p>
                  Недавно у нас вышел сингл “Навсегда к тебе”, о том,
                  что каждый может прийти к своему счастью.
                </p>
                <p>
                  И чтобы подчеркнуть это, мы предлагаем тебе бежать
                  вместе с нами.
                </p>
                <p>
                  Каждое прослушивание — дополнительные 608 метров.
                </p>
              </div>
            ...
            <div className="relative mx-auto flex h-[340px] w-[340px] shrink-0 items-center justify-center">
              ...
              >
                ...
                <circle
                  ...
                  style={{
                    filter:
                      "drop-shadow(0 0 6px rgba(11,93,136,0.4))",
                  }}
                />
              </svg>
        ...
        <section className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-white rounded-[24px] border border-[#dbe8ef] px-6 py-6 min-h-[240px] flex flex-col justify-center items-center text-center">
            ...
            <div className="mt-4 text-[52px] font-black leading-none">
              ...
              <span className="ml-2 text-[28px] align-middle">
                чел
              </span>
            </div>
        ...
        <section className="mt-8 flex flex-col md:flex-row gap-5">
          ...
          <div className="flex-1 rounded-[28px] bg-[#B6DAEB] px-7 py-7">
            <p className="text-[18px] font-semibold">
              Подробнее о том, как это работает рассказали в
              видео <br />
              Ну и конечно, ты можешь подписаться на наш Telegram - совсем скоро новый релиз!
            </p>
